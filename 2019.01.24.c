...
pinMode(btnD,INPUT);
  ...
  myservoX.attach(4);  // attaches the servo on pin 9 to the servo object
  myservoY.attach(7);  // attaches the servo on pin 9 to the servo object
...
//Serial.println(valY);
  valY = map(valY, 135, 1018, 179, 0);     // scale it to use it with the servo (value between 0 and 180)
